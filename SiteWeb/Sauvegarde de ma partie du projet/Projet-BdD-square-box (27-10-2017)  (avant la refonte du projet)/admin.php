			...
			if (isset($_SESSION['admin']) AND $_SESSION['admin'] == 'TRUE') // Si user est admin
			{
				if (isset($_GET['type']) AND $_GET['type'] == 'membres') // Si le type de l'entrée est membres
				{
					...
					if (isset($_REQUEST['id_suppr']) AND !empty($_REQUEST['id_suppr'])) // Supprimer un compte
					{
						...
						$req_suppr = $bdd-> prepare('DELETE FROM membres WHERE id_user = ? ');
						$req_suppr-> execute(array($id_suppr));
						header('location: admin.php');
					}
				}
...
<html>
	...
	<body>
			...
			<div align='center'> <br>
				<h2 class="explic"> Espace d'administration : </h2>

			<?php

				echo '<font color=\'red\' size=\'4\'> Vous êtes connecté sous le compte de ' . $user_info['pseudo'] . '</font> <br> --> <a class=\'link\' href=\'disconnect.php\'> Se déconnecter </a>';

				echo '</div> </ul> <div align=\'left\'>';
...
//____________________________________________________________________________________________________
				while ($u = $user->fetch())
				{
					...
					{
						echo '&nbsp;&nbsp; --> <a class=\'link\' href="admin.php?type=membres&id_confirm=' . $u['id_user'] . '"> Confirmer</a> <br>';
					}
					elseif ($u['confirm_by_mail'] == 'TRUE')
					{
						echo '<font class=\'important\'> &nbsp;&nbsp; Compte confirmé ! </font> <br>';
					}

					if ($u['admin'] == 'FALSE')
					{
						echo '&nbsp;&nbsp;&nbsp; --> <a class=\'link\' href="admin.php?type=membres&id_admin=' . $u['id_user'] . '"> Passer admin</a> <br>';
					}
					elseif ($u['admin'] == 'TRUE')
					{
						echo '<font color=red> &nbsp;&nbsp; Compte admin ! </font>';
						echo '&nbsp;&nbsp;&nbsp; ==> <a class=\'link\' href="admin.php?type=membres&id_revoq_admin=' . $u['id_user'] . '"> Révoquer les droits Admin </a> <br>';
					}
					
					echo '&nbsp;&nbsp; --> <a class=\'link\' href="admin.php?type=membres&id_suppr=' . $u['id_user'] . '" onclick="return(confirm(\'Vous allez supprimer ce membre. Attention, cette action est irréversible !\'));"> Supprimer </a> </li> <br>';
...
//____________________________________________________________________________________________________
		...
		{ ?>
			...
			$erreur = 'Vous n\'êtes pas connecté ! <br> Connectez-vous en temps qu\'admin pour accéder à cette page !';
		...
		if (isset($erreur))
		{
			echo '<div align=\'center\'> <font color=\'red\' size=\'5\'> <br> <br> <br> <br> <br> <br> <br> <br>' . $erreur . '</font> </div>';
		}
