...
//__Connex Android a la BdD___________________________________________________________________________________________
		...
		if (isset($_REQUEST['user'])) // Exemple : android.php?user={"pseudo":"ioio","motdepasse":"42"}
		{
			$connex_android = json_decode($_REQUEST['user'], TRUE);

			$pseudo_android = $connex_android['pseudo']; //user = ioio

			$motdepasse_android = $connex_android['motdepasse']; // mdp = 42

			$motdepasse_android = sha1($motdepasse_android); // mdp = 92cfceb39d57d914ed8b14d0e37643de0797ae56
...
//-----------------------------------------------------------------------------------------------------
			...
			{
				if ($value['pseudo'] == $pseudo_android AND $value['motdepasse'] == $motdepasse_android)
				{
...
//__Récupération des infos de l'apart___________________________________________________________________________________________
		...
		if (isset($_REQUEST['infos'])) // android.php?infos={"id":"102555","idl":"A1","adresse1":"458","adresse2":"5896","ville":"grenoble","codepostal":"25896","date":"10/02/0255","heure":"12:10","prix":"55","commentaire":"srgfdg", "info":"modifier","dimension":"123"}
		{
...
//___Envoi des données à liam__________________________________________________________________________________________
		...
		{
			$call_android = json_decode($_REQUEST['call'], TRUE);
			$data_android = $call_android['data'];

			if ($data_android == 'send')
			{
				...
				$req_pic = $bdd-> prepare("SELECT pic_id_user, pic_idl, pic_path FROM informations WHERE pic_path IS NOT NULL");
				$req_pic-> execute();
				$photo_path = $req_pic-> fetchAll(PDO::FETCH_ASSOC);
				$nb_of_pics = $req_pic-> rowcount();
				//var_dump($photo_path);

				//$path = "http://<IP_PUBLIC>/Projet-BdD-square-box/fichiers/membres/photo_apart/";
				//path quand PC connecté sur la 4g de mon téléphone
					...
					if ($actual_key < $nb_of_pics)
					{
						...
						switch ($nb_row)
						{
							case '1':
								...
								elseif ($str_path['0'] != '') //si qqch dans première ligne 
								{
									array_push($send_img, array(
										'url_img1' => $path . $str_path['0'],
										'url_img2' => $default,
										'url_img3' => $default,
										'url_img4' => $default,
									));
				...
				$arr_test = json_encode($arr_test);
				var_dump($var_test);
				echo $var_test-> rowcount();*/
				//var_dump($send_img);
